...
function Base.setproperty!(Δ::GrassmannTangent, sym::Symbol, v)
    return error("type GrassmannTangent does not allow to change its fields")
end

# Basic vector space behaviour
function Base.:+(Δ₁::GrassmannTangent, Δ₂::GrassmannTangent)
    return GrassmannTangent(checkbase(Δ₁, Δ₂), Δ₁.Z + Δ₂.Z)
end
function Base.:-(Δ₁::GrassmannTangent, Δ₂::GrassmannTangent)
    return GrassmannTangent(checkbase(Δ₁, Δ₂), Δ₁.Z - Δ₂.Z)
end
